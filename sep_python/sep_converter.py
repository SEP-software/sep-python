...
class Dtype_convert:
  ...
  def add_data_type(self,local_name:str,sep_name:str,old_style_name:str,numpy_type:type,esize:int):
    """
      Add a datatype
    ...
    """
    self._numpy_to_name[numpyT]=local_name
    self._name_to_numpy[local_name]=numpy_T
  ...
  def sep_name_to_numpy(self,SEP_name:str)->type:
    ...
    if SEP_name not in self._sep_to_name:
      self._logger.fatal(f"Invalid sep type {SEP_name} {self._SEP_to_name}")
      raise Exception("")
  ...
  def get_size(self,local_name)->int:
    """
    Return esize given local name
    """
    return self._name_to_esize[self.get_name(local_nae)]
