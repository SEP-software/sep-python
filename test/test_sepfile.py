...
def test_simpleWriteHistoryGCS():
    hyper=hypercube(ns=[10,12],os=[2.,1.])
    fle=sepPython.sepIO.sGcsObj(hyper=hyper,path="gs://scratch-sep/junk.H",type="float64")
    fle.close()
    fin=sepPython.sepIO.sGcsObj(path="gs://scratch-sep/junk.H")
    assert 10==fin.getInt("n1")
    assert 12==fin.getInt("n2")
    assert .01 > abs(fin.getFloat("o1")-2.)
    fin.remove()
...
def test_simpleWriteReadGCP():
    ...
    for j in range(ar.shape[0]):
        for i in range(ar.shape[1]):
            ar[j,i]=100*j+i
    fle=sepPython.sepIO.sGcsObj(array=ar,path="gs://scratch-sep/junk2.H")
    fle.writeDescription()
    fle.write(ar)
    fle.close()

    fl2=sepPython.sepIO.sGcsObj(path="gs://scratch-sep/junk2.H")
    ar2=np.zeros((20,10))
    fl2.read(ar2)
    for j in range(ar.shape[0]):
        for i in range(ar.shape[1]):
